...
- Catálogos (`lib/sim/desarrollo/catalogos.ts`): `generarCatalogos` = ratio promedio del corpus × base de Juárez (m² construidos del cuadro aprobado, llaves, niveles o por hotel); P.U. = promedio de precios a 2022 × factor; confianza por dispersión relativa del ratio (máx−mín)/promedio: alta < 10%, media < 25%, baja en otro caso o con un solo hotel. Cuatro conceptos propios del sitio (abatimiento del NAF, retiro de relleno, cinta calefactora, bomba de calor) no tienen referencia: P.U. "Por cotizar" y van a la cola "Requiere revisión". Excel real con una hoja por catálogo (`filasExcel`). En el corpus, las cantidades enteras por llave o por hotel no varían entre hoteles.

...
## Prototipo Desarrollo hotelero (/desarrollo)
Asistente de proyecto ejecutivo y motor de auditoría sobre un corpus de 5 hoteles. Perfiles (`PerfilDesarrollo`): direccion, revisor, proyectista. Documento completo: `../03-prototipo-desarrollo-hotelero.md`.
- Tipos en `lib/types/desarrollo.ts` (HotelCorpus, Proyecto, Fuente, Hallazgo, etc.). Todo dato generado lleva `fuentes: Fuente[]` (corpus: hotel, documento, página o clave de plano; input: inputId); la UI siempre muestra "Fuente" junto al dato.
- El score de auditoría se calcula solo con hallazgos no descartados: por rubro, 100 − (críticos×25 + medios×10 + menores×3) acotado a 0..100, ponderado por pesos coordinacion 30, marca 20, funcional 15, constructiva 20, documental 10, economica 5. IDs de hallazgo: C-nn críticos, M-nn medios, N-nn menores.
- Las vistas bajo /desarrollo/proyectos/*/criterios, /completitud y /catalogos deben funcionar en tablet 1024×768.
- Nada en este prototipo "genera planos": la carga del paquete ejecutivo siempre la hace el proyectista.
- Corpus (`lib/fixtures/desarrollo/corpus/`): un archivo por hotel con `construirHotel` (construir.ts). Los catálogos salen de la lista maestra `conceptos.ts` (5 catálogos, 30–35 conceptos) con variación determinista por hotel y concepto (dispersión b/m/a) y precios deflactados a la fecha de origen (2018–2022). Guaymas no tiene Instalaciones (sin `mep` y solo catálogo de obra civil); Cancún tiene Interiores parcial. Algunos conceptos solo existen en un hotel (`soloEn`).
- Los archivos del corpus y `lib/types/desarrollo.ts` no usan alias `@/` e importan con extensión `.ts` (`allowImportingTsExtensions`): los importa `scripts/desarrollo/generar-textos.mts`.
- Textos del corpus: `redaccion.ts` redacta las memorias (arquitectónico 3 págs., estructural 2, instalaciones 3, interiores 2) y el catálogo (una página por catálogo) desde los datos del hotel; `pnpm gen:desarrollo` escribe `textos/<hotel>-<disciplina>.txt` y `textos.json`. Si cambian los hoteles o la redacción, regenerar: hay prueba que compara `textos.json` con la redacción. Las secciones caen en las páginas de `PAGINA` (construir.ts), que citan las fuentes.
- Consultas del corpus (`corpus/consultas.ts`): 12, con respuesta y tabla calculadas de los datos (no escritas a mano) y fuentes a la página de la memoria.
- Proyecto Juárez (`lib/fixtures/desarrollo/proyecto.ts`): 128 llaves, fase 1, faltan mecánica de suelos (obligatorio) y CAPEX objetivo (complementario). El cuadro objetivo se define contra el promedio del corpus: áreas públicas −20% (la mayor desviación), BOH +10%, estacionamiento +12%. Marca (20 requisitos), decisiones (8), riesgos (10) y entregables (13, suman 100) en sus fixtures.
- Store `lib/store/desarrollo.ts` (clave 'desarrollo', `VERSION_DATOS_DESARROLLO` con migrate; el corpus no se persiste). Fases: 1 Inputs → 2 al liberar el gate (último obligatorio cargado; retirar uno regresa a 1) → 4 al aprobar la definición (`aprobarGateDefinicion`, solo desde 2–3, genera acta) → 5 al ejecutar la auditoría. Cada acción deja evento en `bitacora`.
- `components/desarrollo/DesarrolloShell.tsx`: navegación por perfil (`INICIO_DESARROLLO`: dirección → corpus; revisor y proyectista → tablero) y acceso por ruta con la regla del prefijo más largo (`puedeVerDesarrollo`), según la tabla de rutas del documento. La Biblioteca es su propia ruta (`/desarrollo/proyectos/juarez/biblioteca`) para que el proyectista la tenga en la navegación.
- Corpus (`CorpusDesarrollo`): tarjetas por hotel con matriz de cobertura (lo completo discreto; parcial y ausente resaltados) y panel "Estado del corpus".
- Consulta (`lib/sim/desarrollo/consulta.ts`): índice minisearch de `textos.json` por párrafo (en catálogos, un concepto por fragmento); `consultar` usa la consulta precomputada si `similitud` ≥ 0.6, si no "Resultado por texto". `VisorDocumentoCorpus` muestra el texto de origen en la página citada.
- Búsqueda compartida en `lib/texto.ts` (`normalizar`, `terminos`, `palabras`, `similitud`, `extracto`) y `components/shared/Resaltado.tsx`; la usan Contratos y Desarrollo.
- `DemoBar` muestra "Elegir perfil" si el perfil guardado es de otro prototipo (el perfil de demo es compartido).
- Inputs de ejemplo: `pnpm gen:desarrollo-inputs` (pdf-lib + `zip`) genera `public/fixtures/desarrollo/inputs/*.pdf` (anteproyecto de 3 láminas con plantas esquemáticas, uso de suelo, mecánica de suelos 18 t/m² y NAF a 6 m, topografía, brand standards de 56 págs. y reglamento de 20 con el contenido en las páginas que citan las fuentes, programa, estudio de mercado, CAPEX objetivo) y `terreno.kmz`. Sus datos salen de `lib/fixtures/desarrollo/anteproyecto.ts` y `terreno.ts` (sin alias), los mismos que usa la app. Evitar "−", "≥" y "≤" en textos de PDF (Helvetica WinAnsi).
- Gates (`lib/sim/desarrollo/gates.ts`): `evaluarInputs` → nivel 1 (obligatorio ausente, con impacto), nivel 2 (complementario ausente, supuesto y entregables en ámbar) o libre. Estado de fases y módulos del tablero en `fases.ts` (fase 2 del store = Retrieval completo y Validation en curso).
- Benchmark (`benchmark.ts`): promedio y rango por zona, desviación y semáforo ±8/±15%. CAPEX (`capex.ts`): costo por m² por disciplina de los catálogos (precios a 2022 con 5.5% anual × factor configurable, tipo de cambio 18.5), repartido por zona con pesos; estacionamiento aparte; acabados, FF&E e indirectos como supuesto paramétrico (sin catálogo en el corpus); rango ±10%; ámbar sin CAPEX objetivo, y con objetivo verde/ámbar/rojo según la desviación. Hoy da ~USD 50k por llave contra un objetivo de 46k (+8%, ámbar).
- Fase de Definición (`components/desarrollo/definicion/`): tabs de cuadro de áreas (ajuste con recálculo y restaurar; bloqueado tras aprobar), marca (el revisor corrige estatus), decisiones, riesgos (el revisor confirma o descarta) y CAPEX (la Dirección ajusta el factor). "Aprobar Fase de Definición" solo para el revisor: acta con resumen congelado → fase 4. `Fuentes` (con `ProveedorFuentes` en el shell) abre el visor del corpus en la página citada o el PDF del input en `#page=N`. Semáforo con ícono y texto (`IndicadorSemaforo`).
- Criterios (`lib/fixtures/desarrollo/criterios.ts`): 5–6 por disciplina; los rangos numéricos (kW, TR, L/s por llave, m³ y kg por m², m² por llave) se calculan del corpus. Vista `CriteriosDisciplina` con "Paquete de criterios" (soluciones agregadas) y exportación por impresión (`lib/imprimir.ts`, compartido con el PDF del DataGrid; `esc()` para interpolar texto).
- Biblioteca (`lib/fixtures/desarrollo/biblioteca.ts`, sin alias): 30 soluciones con hotel y clave de origen; cada una tiene su lámina en `public/fixtures/desarrollo/biblioteca/<id>.pdf` que se abre en `VisorPdf`. Ruta propia `/biblioteca` con búsqueda sin acentos y filtros de disciplina, área y tipo.
- Completitud (`lib/sim/desarrollo/completitud.ts`): 39 entregables de las secciones 5.2–5.6 con patrón de clave (100–170 AQ, 200 AL, 300 ACW, 700 AH, 750 AN, BIM, PA, ES, IE, IH, PCI, HV, DI, MEM-*); `verificar` los empareja por nombre; `puedeAuditar` exige Fase 04 y paquete sin faltantes críticos, salvo `notaSupuestoPaquete` (cambiar el paquete la borra). Paquete de ejemplo en `lib/fixtures/desarrollo/paquete.ts` (60 archivos, sin MEM-ES ni IE-002, 3 sin clave) con PDFs en `public/fixtures/desarrollo/paquete/`. La carga es multiarchivo y solo guarda nombre y tamaño.
- Módulos de Fase 04 (criterios, biblioteca, completitud) muestran `RequiereFase` si la definición no está aprobada.
- Catálogos (`lib/sim/desarrollo/catalogos.ts`): `generarCatalogos` = ratio promedio del corpus × base de Juárez (m² construidos del cuadro aprobado, llaves, niveles o por hotel); P.U. = promedio de precios a 2022 × factor; confianza por dispersión relativa del ratio (máx−mín)/promedio: alta < 10%, media < 25%, baja en otro caso o con un solo hotel. Cuatro conceptos propios del sitio (abatimiento del NAF, retiro de relleno, cinta calefactora, bomba de calor) no tienen referencia: P.U. "Por cotizar" y van a la cola "Requiere revisión". Excel real con una hoja por catálogo (`filasExcel`): "Exportar <catálogo> a Excel" en cada pestaña y "Exportar los 5 catálogos" en el encabezado. En el corpus, las cantidades enteras por llave o por hotel no varían entre hoteles.
- `DataGrid` acepta `exportable={false}` para ocultar su exportación cuando la vista tiene la suya.
- Semáforo (`SemaforoTrazabilidad`): el revisor califica; `sugerencias` (en `trazabilidad.ts`) propone verde/ámbar/rojo con motivo (desviaciones, % de marca, riesgos pendientes, CAPEX contra objetivo, completitud por disciplina, conceptos por revisar, auditoría) y "Aplicar sugerencias". `trazabilidad` arma dato → fuentes para los 13 entregables; "Exportar expediente de trazabilidad" imprime todo.
- Auditoría (`lib/sim/desarrollo/auditoria.ts`): `pasosAuditoria` da 10 pasos de ProgressRunner (25 s: Inputs, Indexation, Audit engine A–F por rubro, Reporting, QA / Cierre); los pasos `motor-<rubro>` revelan en vivo los hallazgos de su rubro. `calcularScore` aplica la fórmula de arriba (verde ≥ 85 Liberar, ámbar ≥ 65 Corregir, rojo Replantear) y `resumenEjecutivo` redacta el resumen desde los conteos. Hallazgos en `lib/fixtures/desarrollo/hallazgos.ts` (`crearHallazgos`): coordinación con 2 críticos para que descartar C-03 (nivel 2) suba el score de 36.1 a 43.6; hay prueba.
- `/auditoria` (`AuditoriaProyecto`): resumen del gate documental y "Ejecutar auditoría" (solo el revisor, con `puedeAuditar`); al terminar guarda con `ejecutarAuditoria` y navega al reporte. `/auditoria/reporte` (`ReporteAuditoria`): score, desglose por rubro y tabs (resumen, matriz con Confirmar / Ajustar / Descartar con motivo para el revisor, clash report con `PlanoEsquematico` por nivel, marca, top 10 constructivos, plan de acción con resolución editable y Excel). "Exportar reporte" imprime con portada.
- `PlanoEsquematico`: SVG con ejes A–H × 1–12; nivel 1 ocupa A–H y la torre C–H; marcadores por severidad (forma + texto en el tooltip); los descartados se atenúan.
- El proyectista tiene "Reporte de auditoría" en la navegación (actualiza la resolución del plan de acción). El reporte muestra la nota de supuesto del paquete si la auditoría se hizo con ella.
- Guion de validación (sección 7) cubierto por `lib/store/guion-desarrollo.test.ts`: gate con mecánica de suelos, ajuste de áreas públicas, riesgos y aprobación, biblioteca, paquete con faltantes y supuesto, catálogo eléctrico, auditoría con C-03 descartado y "Reiniciar demo" → fase 1.
- En efectos de React usar cuerpo con llaves: `scrollIntoView` devuelve una promesa en Chrome reciente y, si el efecto la regresa, React la toma como limpieza y truena al desmontar.
